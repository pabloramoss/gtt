    ...
    <div style={{background: theme.colors.graylight}} className='h-screen w-screen absolute top-0 left-0 z-10'>
      ...
      <div className='absolute h-screen w-screen top-0 left-0 z-20'>
        <div className='flex items-center justify-center h-full'>
          <img onClick={handleClick} style={{ opacity, transition: "opacity 1s ease-in-out" }} src="/ticket-convalidato.png" className='p-7' />
        </div>
      </div>
      {isLoading && (
        ...
        {isLoading && <div style={{ opacity: opacity*.6, transition: "opacity 1s ease-in-out" }} className='absolute h-screen w-screen bg-black opacity-60 z-20 top-0 left-0'></div>}
    </div>
  )
}
